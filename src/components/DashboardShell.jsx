...
export default function DashboardShell({ title, children }) {
  ...
  // Close the top-bar user dropdown on outside click.
  useEffect(() => {
        ...
        <header className="flex items-center gap-4 bg-white/80 backdrop-blur-xl border-b border-[#E2E8F0] px-4 md:px-6 py-3 flex-shrink-0 z-10">
          ...
          <div className="hidden md:block relative flex-shrink-0" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[#F1F5F9] transition"
            >
              <span className="inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[#C0272D] text-[11px] font-bold text-white uppercase">
                {initials(displayName)}
              </span>
              ...
              <ChevronDown
                className={`h-4 w-4 text-[#94A3B8] transition-transform ${userMenuOpen ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {userMenuOpen && (
              <div
                role="menu"
                className="absolute right-0 mt-2 w-44 rounded-lg border border-[#E2E8F0] bg-white shadow-lg py-1 z-20 animate-fade-in"
              >
                <button
                  role="menuitem"
                  onClick={() => { setUserMenuOpen(false); signOut() }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#0F172A] hover:bg-[#F1F5F9] transition"
                >
                  <LogOut className="h-4 w-4 text-[#64748B]" strokeWidth={1.9} />
                  Sign out
                </button>
              </div>
            )}
