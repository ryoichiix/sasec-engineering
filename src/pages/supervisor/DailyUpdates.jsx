...
export default function DailyUpdates() {
  ...
  const postUpdate = async () => {
    ...
    if (newAtts.length) {
      setAttachmentsByUpdate((prev) => ({ ...prev, [update.id]: newAtts }))
    }
    setNewText('')
    setNewFiles([])
    setVoiceFile(null)
  ...
  const [formVoice, setFormVoice] = useState({}) // keyed by field name
  const [formVoiceKey, setFormVoiceKey] = useState(0) // increment to reset all VoiceRecorders
  const [submitting, setSubmitting] = useState(false)
  const [submitError, setSubmitError] = useState(null)

  // Imperative prefill (not via effect) avoids the set-state-in-effect lint rule.
  const startEditing = () => {
    if (report) {
      setForm({
